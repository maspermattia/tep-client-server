...
public class Game implements Runnable {
    ...
    public void run() {
        try {
            ...
            while (true) {
                in.readLine(); // Attendi conferma dal client per iniziare la partita
                distribuisciCarte();
                gestisciGiocatori();
                gestisciMazziere();
                determinaVincitori();
    ...
    private int pescaCarta() {
        // Simuliamo l'estrazione di una carta
        int valoreCarta = (int) (Math.random() * 10) + 1; // Valore casuale da 1 a 10
        if (valoreCarta > 7) {
            valoreCarta = 7; // Per semplificare, consideriamo il massimo valore possibile come 7
        }
        return valoreCarta;
    }
